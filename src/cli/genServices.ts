...
const header = `/**${NEW_LINE} * Auto generated by Dubbo2Ts.${NEW_LINE} *${NEW_LINE} * ${new Date().toString()}${
    NEW_LINE
} */${NEW_LINE_2}`;
...
const genService = (pkg, serviceName, serviceAst, group: string, version: string, timeout: number): IServiceCode => {
    ...
    const methodHandler = (name, func) => {
        ...
        methodCode.push(
            `export function ${name}(${args.map(arg => arg.name + ': ' + mapType(arg.type)).join(',')}) {${NEW_LINE}`
        );
...
export default { genServices, genIndexFile };
