...
var Dubbo = /** @class */ (function () {
    function Dubbo(options) {
        ...
        this.applyServices = function () {
            for (var key in _this.services) {
                _this.serviceImpls[key] = new Service_1.default(_this.client, _this.dubboVer, _this.services[key], _this);
                if (++COUNT === SERVICE_LENGTH) {
                    console.log("\x1b[32m%s\x1b[0m", "[" + _this.application + "]Dubbo service init done");
                }
            }
        };
    ...
    Dubbo.exec = function (serviceNMethod, payload) {
        if (!Dubbo.instance) {
